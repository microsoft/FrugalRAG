...
from .data.data_utils import format_input_context, load_corpus
...
class CustomReAct(dspy.ReAct):
    ...
    def get_next_hop(self, **input_args):
        ...
        try:
            ...
            while True:
                if compute_trajectory_tokens(trajectory, self.tokenizer) > MODEL_MAX_LEN and (self.args.model.max_iters > 5 or self.args.data.dataset_name in ["bcplus", "researchy"]):
                ...
                try:
                    ...
                    if lm_query is not None:
                        with dspy.context(lm_query=lm_query):
                            observation = self.tools[tool_name](pred.next_tool_args.get("search_query")) if tool_name == "AdvancedSearch" else self.tools[tool_name](**pred.next_tool_args)
                    else:
                        observation = self.tools[tool_name](pred.next_tool_args.get("search_query")) if tool_name == "AdvancedSearch" else self.tools[tool_name](**pred.next_tool_args)
                    break
    ...
    def forward(self, **input_args):
        ...
        if input_args.get("get_original_traj"):
...
class AnswerOnly(dspy.ReAct):
    def __init__(self, signature):
        ...
        self.react = dspy.Predict("a -> b")  # dummy signature
        self.extract = dspy.ChainOfThought(fallback_signature)

    def forward(self, **input_args):
        trajectory = input_args.pop("trajectory", None)
        assert trajectory is not None
        extract = self.extract(**input_args, trajectory=format(trajectory))
