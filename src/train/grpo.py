...
import random
from src.data.truncation_utils import adaptive_truncate_for_react
...
class GRPOReActTrainer(GRPOTrainer):
    ...
    def _generate_and_score_completions(
        ...
        for hop in range(self.max_iters):
            ...
            # Add truncation after creating prompts_text:
            prompts_text = self._truncate_prompts_if_needed(prompts_text, trajectories) if self.max_iters > 5 else prompts_text
    ...
    def _truncate_prompts_if_needed(self, prompts_text, trajectories=None):
        MODEL_MAX_LEN=20000
        """Adaptively truncate prompts that are too long"""
        truncated_prompts = []
        
        for i, prompt in enumerate(prompts_text):
            # Check if prompt is too long
            prompt_tokens = len(self.processing_class.tokenize(prompt))
            
            if prompt_tokens > MODEL_MAX_LEN:
                print(f"[INFO] Prompt {i} too long ({prompt_tokens} tokens), truncating...")
                
                # If we have trajectories, truncate them first
                if trajectories and i < len(trajectories):
                    truncated_trajectory = adaptive_truncate_for_react(
                        trajectories[i], 
                        tokenizer=self.processing_class,
                        preserve_recent=2, 
                        max_seq_length=MODEL_MAX_LEN - 1000
                    )
                    
                    # Recreate the prompt with truncated trajectory
                    react_input = {
                        "prompt": self.adapter.format(
                            signature=self.react_model.react.signature,
                            demos=self.react_model.react.demos,
                            inputs={
                                "question": trajectories[i].get("question", ""),
                                "trajectory": format_trajectory(truncated_trajectory),
                            },
                        )
                    }
                    truncated_prompt = maybe_apply_chat_template(react_input, self.processing_class)["prompt"]
                    truncated_prompts.append(truncated_prompt)
                else:
                    # Fallback: simple truncation
                    tokens = self.processing_class.tokenize(prompt)
                    truncated_tokens = tokens[-(MODEL_MAX_LEN - 1000):]  # Keep recent tokens
                    truncated_prompt = self.processing_class.decode(truncated_tokens, skip_special_tokens=True)
                    truncated_prompts.append(truncated_prompt)
            else:
