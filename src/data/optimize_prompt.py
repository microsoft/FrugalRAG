...
def generate_fewshot(args, lm):
    if not (os.path.exists(args.paths.prompt_path) and len(os.listdir(args.paths.prompt_path)) > 3):
        ...
        num_candidate_programs = 3 * candidates if args.data.dataset_name == "bcplus" else 5 * candidates

        pipeline = Hopper(args, custom=True, debug=False)
        config = {
            "max_bootstrapped_demos": 1 if args.data.dataset_name=="bcplus" else 3,
            "num_threads": 32,
