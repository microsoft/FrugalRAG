...
async def server_loop(q):
    searcher: E5Searcher = E5Searcher(
        index_dir=os.getenv(
            "INDEX_DIR", "/mnt/ddn/alta01/t-abjava/data/data/e5-large-index/"
        ),
        model_name_or_path=os.getenv(
            "E5_MODEL_NAME_OR_PATH", "intfloat/e5-large-v2"
        ),
        single_index_file=os.getenv(
            "INDEX_FILE", ""
        )
    )
    top_k = int(os.getenv("TOP_K", 5))*50
    logger.info(f"E5Searcher initialized, ready to serve requests, top_k={top_k}")
    # warmup the server a bit
    searcher.batch_search(
        queries=[f"test query {random.random()} {i}" for i in range(4)], k=top_k
    )
    ...
    while True:
        ...
        try:
            results_list: List[List[Dict]] = searcher.batch_search(
